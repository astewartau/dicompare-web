...
const AcquisitionSelector: React.FC<AcquisitionSelectorProps> = ({
  schema,
  onSelectAcquisition,
  onBack
}) => {
  ...
  if (acquisitions.length === 0) {
    return (
      <Box textAlign="center" p={6}>
        ...
        <Button onClick={onBack} colorScheme="gray">Back to Library</Button>
      </Box>
    );
  }
      ...
      {schemaData.authors && schemaData.authors.length > 0 && (
        <Flex mb={4} gap={2} alignItems="center">
          ...
          {schemaData.authors.map((author: string, idx: number) => (
            <Badge key={idx} colorScheme="blue">{author}</Badge>
          ))}
        </Flex>
      )}
