  ...
  return (
    <Box p={8}>
      ...
      <Box p={4} borderWidth="1px" borderRadius="md" bg={statusColor} mb={6}>
        ...
        <Text fontSize="sm" mt={2}>Certification level: <Badge colorScheme={certificationLevel === "Gold" ? "yellow" : certificationLevel === "Silver" ? "gray" : "orange"}>{certificationLevel}</Badge></Text>
        <Text fontSize="sm" mt={2}>Checked {results.checkedDICOMS} DICOMs</Text>
      ...
      {/* Errors Section */}
      <Box p={4} borderWidth="1px" borderRadius="md" bg="red.50" overflow="auto" maxHeight="300px" minWidth="100vh" mb={6}>
        <Heading as="h3" size="sm" color="red.500" mb={4}>Errors</Heading>
        {results.errors.map((error, index) => (
          <Box key={index} mb={4}>
            <HStack justify="space-between" align="center">
              <Text fontSize="xs" fontWeight="bold">{error.field}</Text>
              <IconButton icon={expandedErrors.includes(index) ? <ChevronUpIcon /> : <ChevronDownIcon />} size="sm" onClick={() => toggleExpand(index, "error")} aria-label="Toggle error details" />
            </HStack>
            <Collapse in={expandedErrors.includes(index)} animateOpacity>
              <Box mt={2} pl={4}>
                <Text fontSize="xs"><strong>Expected:</strong> {error.expected}</Text>
                <Text fontSize="xs"><strong>Actual:</strong> {error.actual}</Text>
      ...
      <Box p={4} borderWidth="1px" borderRadius="md" bg="green.50" overflow="auto" maxHeight="300px">
        <Heading as="h3" size="sm" color="green.500" mb={4}>Compliant Fields</Heading>
        {results.compliantFields.map((field, index) => (
          <Box key={index} mb={4}>
            <HStack justify="space-between" align="center">
              <Text fontSize="xs" fontWeight="bold">{field.field}</Text>
              <IconButton icon={expandedCompliant.includes(index) ? <ChevronUpIcon /> : <ChevronDownIcon />} size="sm" onClick={() => toggleExpand(index, "compliant")} aria-label="Toggle compliant field details" />
            </HStack>
            <Collapse in={expandedCompliant.includes(index)} animateOpacity>
              <Box mt={2} pl={4}>
                <Text fontSize="xs"><strong>Value:</strong> {field.value}</Text>
      ...
      <Box p={4} borderWidth="1px" borderRadius="md" bg="gray.50" overflow="auto" maxHeight="300px" mb={6}>
        <HStack justify="space-between">
          ...
          <Button leftIcon={<CopyIcon />} size="sm" onClick={onCopy}>{hasCopied ? "Copied" : "Copy JSON"}</Button>
        </HStack>
        <Code as="pre" fontSize="sm" bg="gray.100" p={2} borderRadius="md" whiteSpace="pre-wrap" wordBreak="break-word">{JSON.stringify(results, null, 2)}</Code>
      </Box>
      <Button colorScheme="green" onClick={onNext} mt={4}>Generate Signed Report</Button>
    </Box>
  );
};
