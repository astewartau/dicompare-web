...
import { Box, Text, Flex, IconButton, Collapse, Tooltip, VStack, Button } from '@chakra-ui/react';
import {
    ChevronDownIcon,
    ChevronRightIcon,
    CheckCircleIcon,
...
const PairingArea: React.FC<PairingAreaProps> = ({
    ...
    const renderCard = (acq: Acquisition, type: 'ref' | 'inp') => {
        const expanded = (type === 'ref' ? expandedReferences[acq.id || acq.name] : expandedInputs[acq.name]) || false;
        ...
        return (
            <Box borderWidth="1px" borderRadius="md" mb={1}>
                <Box
                    p={2}
                    display="flex"
                    ...
                    <Box p={2} bg="gray.50">
                        {acq.details.rules ? (
                            <RulesTable
                                ...
                                schemaId={acq.id} // Pass the schema ID
                            />
                        ) : (
                            <>
                                {acq.details.fields && (
